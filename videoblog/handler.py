...
db_connection = psycopg2.connect(
    dbname=DATABASE, user=PG_USER, password=PG_PASSWORD, host=ip)
...
async def help(call: CallbackQuery):
    ...
    city = str(call.data)
    cursor.execute(
        f"SELECT CASE WHEN EXISTS(SELECT user_id FROM us_id WHERE user_id = '{user_id}') THEN 1 ELSE 2 END;")
    row = (cursor.fetchone())[0]
    if row == 2:
        cursor.execute(
            f"INSERT INTO us_id(user_id,city,message_id) VALUES ('{user_id}', '{city}', '{message_id}');")
        db_connection.commit()
        if call.data == 'Moskva':
...
async def ans(call: CallbackQuery):
    ...
    if call.data == 'support_moskva' and call.from_user.username.lower() in support_name_moscow:
        cursor.execute(
            f"SELECT user_id FROM us_id WHERE city='Moskva' LIMIT 1")
        db_connection.commit()
        z = cursor.fetchall()
        ...
        try:
            cursor.execute(
                f"INSERT INTO support_stats(support_name,user_name, city) VALUES ('{support_name}', '{id}', '{city}');")
        ...
        cursor.execute(
            f"SELECT user_id FROM us_id WHERE city='Irkutsk' LIMIT 1")
        db_connection.commit()
        z = cursor.fetchall()
...
async def word(message: Message):
    if message.from_user.id == admin_id:
        ...
        if msg[0].lower() == 'добавить':
            try:
                cursor.execute(
                    f"INSERT INTO support_list(sup_name,sup_city) VALUES ('{msg[1]}','{msg[2]}');")
                db_connection.commit()
            ...
            try:
                cursor.execute(
                    f"DELETE FROM support_list WHERE sup_name= '{msg[1]}' and sup_city = '{msg[2]}'")
                db_connection.commit()
            ...
            cursor.execute(
                f"SELECT count(*) FROM support_stats WHERE support_name ='{msg[1]}'")
            db_connection.commit()
            count = cursor.fetchone()[0]
...
def st(city):
    sup_list = []
    cursor.execute(
        f"SELECT count(*) FROM support_list WHERE sup_city ='{city}'")
    db_connection.commit()
    count = int(cursor.fetchone()[0])
    cursor.execute(
        f"SELECT sup_name FROM support_list WHERE sup_city='{city}'")
    db_connection.commit()
